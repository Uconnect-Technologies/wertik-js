...
export const generateCrudResolvers = (
  ...
  const beforeBulkDelete = get(
    configuration,
    `events.graphql.${module.name}.beforeBulkDelete`,
    null
  );
  const afterBulkDelete = get(
    configuration,
    `events.graphql.${module.name}.afterBulkDelete`,
    null
  );
  const beforeBulkCreate = get(
    configuration,
    `events.graphql.${module.name}.beforeBulkCreate`,
    null
  );
  const afterBulkCreate = get(
    configuration,
    `events.graphql.${module.name}.afterBulkCreate`,
    null
  );
  const beforeBulkUpdate = get(
    configuration,
    `events.graphql.${module.name}.beforeBulkUpdate`,
    null
  );
  const afterBulkUpdate = get(
    configuration,
    `events.graphql.${module.name}.afterBulkUpdate`,
    null
  );
  // R
  const beforeList = get(
    configuration,
    `events.graphql.${module.name}.beforeList`,
    null
  );
  const afterList = get(
    configuration,
    `events.graphql.${module.name}.afterList`,
    null
  );

  const beforeView = get(
    configuration,
    `events.graphql.${module.name}.beforeView`,
    null
  );
  const afterView = get(
    configuration,
    `events.graphql.${module.name}.afterView`,
    null
  );
