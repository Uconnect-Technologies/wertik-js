...
function default_1({ req, res }) {
    return __awaiter(this, void 0, void 0, function* () {
        const token = lodash_1.get(req, "headers.authorization", false);
        if (token === false) {
            throw internalServerError_1.default({ message: "Unauthorized, Reason: Missing auth token" });
        }
        let user = yield getUserWithAccessToken_1.default(token);
        if (!user) {
            throw internalServerError_1.default({ message: "Unauthorized, You need to signin." });
        }
        return {
            user
        };
